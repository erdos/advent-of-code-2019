...
(defn size [path]
  (assert (vector? path))
  [(reduce + (map second (filter (comp #{:right} first) path)))
   (reduce + (map second (filter (comp #{:down} first) path)))])

(defn man-len [[x y]] (+ (Math/abs x) (Math/abs y)))
...
(defn solve [s1 s2]
  (let [p1 (parse s1)
        ...
        c1 (set (coords p1))
        c2 (set (coords p2))]
    (apply min (map  man-len (clojure.set/difference
...
(defn path-intersections [path1 path2]
    (let [c1 (set (coords path1))
          c2 (set (coords path2))]
      (clojure.set/difference (clojure.set/intersection c1 c2) #{[0 0]})))
...
(let [[p1 p2] (map parse (clojure.string/split-lines (slurp "/home/erdos/input3.txt")))
      ...
      s2 (coords-steps p2)]
  {:day3-second
   (apply min  (for [ci (path-intersections p1 p2)]
